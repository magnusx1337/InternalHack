...
bool screen_transformxx(const Vector& point, Vector& screen)
{
	...
	screen.z = 0.0f;
	float w = w2sMatrix[3][0] * point.x + w2sMatrix[3][1] * point.y + w2sMatrix[3][2] * point.z + w2sMatrix[3][3]; //what the fuck is this retarded freak???
	if (w < 0.001f) {
		screen.x *= 100000;
		screen.y *= 100000;
		return true;
...
void CVisuals::Draw()
{
	...
	if (options::menu.visuals2_tab.OtherNoScope.GetState() && pLocal->IsAlive() && pLocal->IsScoped())
		NoScopeCrosshair();
	if (options::menu.visuals2_tab.AutowallCrosshair.GetState())
	{
		AutowallCrosshair();
