...
void CDropBox::Draw(bool hover)
{
	...
	if (GUI.IsMouseInRegion(Region)) {
		Render::rect(a.x, a.y, m_iWidth, 20, Color(45, 45, 45, MenuAlpha));
		Render::Outline(a.x, a.y, m_iWidth, 20, Color(12, 12, 12, MenuAlpha));
	}
	else {
		Render::rect(a.x, a.y, m_iWidth, 20, Color(45, 45, 45, MenuAlpha));
		Render::Outline(a.x, a.y, m_iWidth, 20, Color(12, 12, 12, MenuAlpha));
	...
	if (items.size() > 0)
	{
		...
		Render::Text2(a.x + 10, a.y + (Region.bottom / 2) - (txtSize.bottom / 2), toDraw.c_str(), Render::Fonts::MenuText, Color(180, 180, 180, MenuAlpha));

		if (IsOpen)
		{
			Render::rect(a.x, a.y + 20, m_iWidth, items.size() * 20, Color(45, 45, 45, MenuAlpha));
			...
			for (int i = 0; i < items.size(); i++)
			{
				...
				{
					Render::rect(a.x, a.y + 20 + i * 20, m_iWidth, 20, Color(36, 36, 36, MenuAlpha));
				}
				else {
					Render::rect(a.x, a.y + 20 + i * 20, m_iWidth, 20, Color(36, 36, 36, MenuAlpha));
				}
				...
				if (!item.GetSelected)
					Render::Text2(a.x + 10, a.y + 20 + (i * 20) + 10 - (control_textsize.bottom / 2), item.text, Render::Fonts::MenuText, Color(245, 245, 245, MenuAlpha));
				else {
					Render::Text2(a.x + 10, a.y + 20 + (i * 20) + 10 - (control_textsize.bottom / 2), item.text, Render::Fonts::MenuText, Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha));
				}

				//Render::Outline(a.x, a.y + 20 + (i * 20), m_iWidth, 20, Color(26, 26, 26, 230));
			}
			Render::Outline(a.x, a.y + 20, m_iWidth, items.size() * 20, Color(12, 12, 12, MenuAlpha));
		}
	}
	Vertex_t Verts2[3];
	...
	Render::Polygon(3, Verts2, Color(120, 120, 120, MenuAlpha));
...
void CCheckBox::Draw(bool hover)
{
	...
	Render::rect(a.x+ 8, a.y-2, 14, 14, Color(28, 28, 28, MenuAlpha));
	Render::outlineyeti(a.x + 8, a.y-2, 14, 14, Color(28, 28, 28, MenuAlpha));

	if (Checked)
	{
		Render::rect(a.x + 2 + 8, a.y, 10, 10, Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha));
		//Render::outlineyeti(a.x, a.y, 10, 10, Color(24, 24, 24, MenuAlpha));
	}
}
...
bool CCheckBox2::GetIsSub()
{
	return bIsSub;
}

void CCheckBox2::SetAsSub(bool t)
{
	bIsSub = t;
}
...
void CCheckBox2::Draw(bool hover)
{
	...
	grad = Color(red, green, blue, 0);




	Render::rect(a.x, a.y, 9, 9, Color(40, 40, 40,MenuAlpha));
	Render::outlineyeti(a.x, a.y, 9, 9, Color(2, 2, 2, MenuAlpha));

	if (Checked)
	{
		Render::rect(a.x, a.y, 9, 9, Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha));
		Render::outlineyeti(a.x, a.y, 9, 9, Color(2, 2, 2, MenuAlpha));
	}
}

void CCheckBox2::OnUpdate() { m_iHeight = 9; }
...
void CGroupBox::Draw(bool hover)
{
	...
	Render::DrawRect(a.x + 2, a.y + 2, m_iWidth - 4, m_iHeight - 4, Color(44, 44, 44, 0));
	...
	if (group_tabs.size())
	{
		...
		Render::rect(a.x + 1, a.y + 36, m_iWidth - 5, 1, Color(100, 100, 100, MenuAlpha));
		...
		for (int i = 0; i < group_tabs.size(); i++)
		{
			...
			if (selected_tab == group_tabs[i].id)
			{
				...
				Render::rect(tab_area.left, a.y + 36, tab_area.right - 1, 1, Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha));
			}
			else if (selected_tab != group_tabs[i].id) {

				Render::Text2(text_position[0] - (text_size.right / 2), text_position[1], group_tabs[i].name.c_str(), Render::Fonts::MenuText, Color(210, 210, 210, MenuAlpha));
			}
		}
	}
	...
	Render::Text(a.x + 15, a.y - (txtSize.bottom / 2), Color(255, 255, 255, MenuAlpha), Render::Fonts::MenuText, Text.c_str());
	Render::Line(a.x, a.y, a.x + 12, a.y, Color(1, 1, 1, MenuAlpha));
	Render::Line(a.x + 15 + txtSize.right + 5, a.y, a.x + m_iWidth, a.y, Color(1, 1, 1, MenuAlpha));
	Render::Line(a.x, a.y, a.x, a.y + m_iHeight, Color(1, 1, 1, MenuAlpha));
	Render::Line(a.x, a.y + m_iHeight, a.x + m_iWidth, a.y + m_iHeight, Color(1, 1, 1, MenuAlpha));
	Render::Line(a.x + m_iWidth, a.y, a.x + m_iWidth, a.y + m_iHeight, Color(1, 1, 1, MenuAlpha));
...
#pragma region GroupBox
CGroupBoxInvisible::CGroupBoxInvisible()
{
	Items = 1;
	last_y = 0;
	m_Flags = UIFlags::UI_Drawable | UIFlags::UI_RenderFirst;
...
void CGroupBoxInvisible::Draw(bool hover)
{
	POINT a = GetAbsolutePos();
	RECT txtSize = Render::GetTextSize(Render::Fonts::MenuText, Text.c_str());
	...
	if (group_tabs.size())
	{
		...
		//Render::Line(a.x + 1, a.y + 38, a.x + m_iWidth, a.y + 38, Color(29, 29, 29, MenuAlpha));
		//Render::rect(a.x + 1, a.y + 9, m_iWidth - 1, 29, Color(20, 20, 19, MenuAlpha));
		Render::rect(a.x + 1, a.y + 36, m_iWidth - 5, 1, Color(100, 100, 100, 0));
		...
		for (int i = 0; i < group_tabs.size(); i++)
		{
			...
			if (selected_tab == group_tabs[i].id)
			{
				...
				Render::Text2(text_position[0] - (text_size.right / 2), text_position[1], group_tabs[i].name.c_str(), Render::Fonts::MenuText, Color(210, 210, 210, 0));
			}
		}
	}
	...
	//	Render::Clear(a.x + 2, a.y + 2, m_iWidth - 4, m_iHeight - 4, Color(90, 90, 90, MenuAlpha));
	Render::Text(a.x + 15, a.y - (txtSize.bottom / 2), Color(255, 255, 255, 0), Render::Fonts::MenuText, Text.c_str());
	Render::Line(a.x, a.y, a.x + 12, a.y, Color(1, 1, 1, 0));
	Render::Line(a.x + 15 + txtSize.right + 5, a.y, a.x + m_iWidth, a.y, Color(1, 1, 1, 0));
	Render::Line(a.x, a.y, a.x, a.y + m_iHeight, Color(1, 1, 1, 0));
	Render::Line(a.x, a.y + m_iHeight, a.x + m_iWidth, a.y + m_iHeight, Color(1, 1, 1, 0));
	Render::Line(a.x + m_iWidth, a.y, a.x + m_iWidth, a.y + m_iHeight, Color(1, 1, 1, 0));

}

void CGroupBoxInvisible::SetText(std::string text)
{
	Text = text;
}

void CGroupBoxInvisible::PlaceLabledControl(int g_tab, std::string Label, CTab * Tab, CControl * control)
{
	bool has_tabs = group_tabs.size() ? 1 : 0;
...
void CGroupBoxInvisible::AddTab(CGroupTab t)
{
	group_tabs.push_back(t);
...
void CGroupBoxInvisible::OnUpdate() {}
void CGroupBoxInvisible::OnClick() {}
...
void CSlider::Draw(bool hover)
{
	...
	Render::rect(a.x, a.y, m_iWidth, 7, Color(55, 55, 55, MenuAlpha));
	...
	Render::rect(a.x, a.y, Location, 7, Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha));
	...
	Render::text_yeti(a.x + Location - (txtSize.right / 2), a.y + 7 - (txtSize.bottom / 2), buffer, Render::Fonts::MenuText, Color(255, 255, 255, MenuAlpha));
}
...
char* KeyStrings[254] = { nullptr, "mouse1", "mouse2", "control+break", "mouse3", "mouse4", "mouse5",
...
void CKeyBind::Draw(bool hover) // just pasted this out from corruption.vip xd i dont like that skeet wannabe keybind x)
{
	POINT a = GetAbsolutePos();

	Render::Outline(a.x + 1, a.y + 1, m_iWidth - 2, m_iHeight, Color(2, 2, 2, MenuAlpha));
	//if (Menu::Window.isOpen())
	//{
		Render::rect(a.x + 2, a.y + 2, m_iWidth - 4, 17, Color(28, 28, 28, MenuAlpha));
	//}
/*	if (Menu::Window.isOpen())
	{
		if (hover)
	...
	char NameBuffer[256];
	char* KeyName = ""; // still nah

	if (Getting_New_Key)
	{
		Render::Text(a.x + 4, a.y + 3 - 1, Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha), Render::Fonts::MenuText, "press a key"); // lets follow supremacy
		//KeyName = "press a key"; 
	}
	else
	{
	...
	Render::Text(a.x + 4, a.y + 3 - 1, Color(150, 150, 150, MenuAlpha), Render::Fonts::MenuText, KeyName);
}
...
void CButton::Draw(bool hover)
{
	POINT a = GetAbsolutePos();
	if (hover)
		Render::gradient_verticle(a.x, a.y, m_iWidth, m_iHeight, Color(5, 5, 5, MenuAlpha), Color(71, 71, 71, MenuAlpha));
	else
		Render::gradient_verticle(a.x, a.y, m_iWidth, m_iHeight, Color(71, 71, 71, MenuAlpha), Color(5, 5, 5, MenuAlpha));
	...
	Render::text_yeti(TextX, TextY, Text.c_str(), Render::Fonts::MenuText, Color(180, 180, 180, MenuAlpha));
}
...
void CComboBoxYeti::Draw(bool hover)
{
	...
	Render::rect(a.x, a.y, m_iWidth, 20, Color(28, 28, 28, MenuAlpha));
	//if (GUI.IsMouseInRegion(Region)) Render::gradient_verticle(a.x, a.y, m_iWidth, 20, Color(45, 45, 45, MenuAlpha), Color(45, 45, 45, MenuAlpha));
	...
	if (Items.size() > 0)
	{
		...
		Render::text_yeti(a.x + 10, a.y + (Region.bottom / 2) - (txtSize.bottom / 2), GetItem().c_str(), Render::Fonts::MenuText, Color(180, 180, 180, MenuAlpha));

		if (IsOpen)
		{
			Render::rect(a.x, a.y + 20, m_iWidth, Items.size() * 20, Color(28, 28, 28, MenuAlpha));
...
void CTextField::Draw(bool hover)
{
	...
	Render::rect(a.x, a.y - 1, m_iWidth, 19, Color(40, 40, 40, MenuAlpha));
	Render::Outline(a.x, a.y - 1, m_iWidth, 19, Color(2, 2, 2, MenuAlpha));
	...
	Render::text_yeti(a.x + 4, a.y, drawn_text.c_str(), Render::Fonts::MenuText, Color(177, 177, 177, MenuAlpha));
}
...
void CTextField2::Draw(bool hover)
{
	...
	Render::Clear(a.x, a.y, m_iWidth, m_iHeight, Color(28, 28, 28, MenuAlpha));
...
void CColorSelector::Draw(bool hover)
{
	...
	Color preview;
	preview.SetColor(color[0], color[1], color[2], MenuAlpha);
...
void CListBox::Draw(bool hover)
{
	...
	Render::rect(a.x + 1, a.y + 1, m_iWidth - 2, m_iHeight - 2, Color(20, 20, 20, 1));
	...
	if (Items.size() > 0)
	{
		...
		for (int i = ScrollTop; (i < Items.size() && drawnItems < ItemsToDraw); i++)
		{
			...
			if (i == SelectedIndex)
			{
				textColor = Color(options::menu.ColorsTab.Menu.GetValue(), MenuAlpha);



				Render::rect(ItemRegion.left, ItemRegion.top, ItemRegion.right, ItemRegion.bottom, Color(37, 37, 37, MenuAlpha));
			}
			...
			Render::text_yeti(ItemRegion.left + 4, ItemRegion.top + 2, Items[i].c_str(), Render::Fonts::MenuText, Color(245, 245, 245, MenuAlpha));
			drawnItems++;
		}
