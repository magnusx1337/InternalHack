...
void CEsp::Draw()
{
	...
	for (int i = 0; i < interfaces::ent_list->GetHighestEntityIndex(); i++)
	{
		...
		if (pEntity &&  pEntity != pLocal && !pEntity->IsDormant())
		{
			...
			if (options::menu.visuals2_tab.FiltersNades.GetState() && strstr(cClass->m_pNetworkName, "Projectile"))
			{
				DrawThrowable(pEntity);
			}
			if (options::menu.visuals2_tab.FiltersWeapons.GetState() && cClass->m_ClassID != (int)CSGOClassID::CBaseWeaponWorldModel && ((strstr(cClass->m_pNetworkName, "Weapon") || cClass->m_ClassID == (int)CSGOClassID::CDEagle || cClass->m_ClassID == (int)CSGOClassID::CAK47)))
			{
				DrawDrop(pEntity, cClass, box);
			}
			if (options::menu.visuals2_tab.FiltersC4.GetState())
			{
				if (cClass->m_ClassID == (int)CSGOClassID::CPlantedC4)
					DrawBombus(pEntity, box);
				if (cClass->m_ClassID == (int)CSGOClassID::CC4)
					DrawBomb(pEntity, cClass);
			}
		}
	}
	if (options::menu.visuals2_tab.OtherNoFlash.GetState())
	{
		float alp = options::menu.visuals2_tab.flashAlpha.GetValue() / 255;
...
void CEsp::DrawBombus(IClientEntity* pEntity, Box size) {
	BombCarrier = nullptr;
	auto entity = pEntity;
	...
	{
		Render::nonamegey(vScreen.x, vScreen.y, "bomb", Render::Fonts::MenuText, Color(255, 255, 255));
		Render::textT(vScreen.x, vScreen.y + 10, Render::Fonts::MenuText, Color(53, 131, 209), "%.1f", lifetime);
	}
	if (interfaces::engine->IsConnected() && interfaces::engine->IsInGame())
	{
		if (lifetime > 0.01f && !Bomb->IsBombDefused())
		{
			...
			Render::gradient_verticle(30, 30, boomval, 8, Color(255, 30, 30, 255), Color(188, 30, 30, 255));
			Render::rect(30, 30, 300, 8, Color(1, 1, 1, 100));
			Render::textT(boomval + 20, 27, Render::Fonts::MenuText, Color(255, 255, 255), "%.1f", lifetime);
			Render::textT(5, height / 2 - 140, Render::Fonts::niggerbomb, Color(255, 10, 10, 255), "%.1fs", lifetime);
		}
	}
	if (interfaces::engine->IsConnected() && interfaces::engine->IsInGame())
	{
		...
		if (lifetime > 0.01f && !Bomb->IsBombDefused() && local->IsAlive())
		{
			if (damage >= local->GetHealth() && lifetime > 0.01f)
			{
				...
				if (lifetime > 0.01f && local->IsAlive()) {
					...
					gey += "HP";
					Render::textT(5, height / 2 - 120, Render::Fonts::niggerbomb, Color(255, 255, 255, 255), gey.c_str());
				}
			}
		}
	}
	...
	if (interfaces::engine->IsConnected() && interfaces::engine->IsInGame())
	{
		if (Bomb->GetBombDefuser() > 0)
		{
			...
			if (countdown > 0.01f)
			{
				...
				{
					Render::gradient_verticle(30, 60, countdown, 8, Color(110, 240, 110, 255), Color(110, 198, 110, 255));
					Render::rect(30, 60, 300, 8, Color(1, 1, 1, 100));
					Render::textT(countdown + 20, 57, Render::Fonts::MenuText, Color(255, 255, 255), "%.1f", countdown);

					Render::textT(5, height / 2 - 100, Render::Fonts::niggerbomb, Color(110, 240, 110, 255), "%.1f", countdown);
				}
			}
		}
	}
...
void CEsp::DrawWeapon(IClientEntity* pEntity, Box size)
{
	...
	if (pWeapon)
	{
		...
		{
			RECT nameSize = Render::GetTextSize(Render::Fonts::MenuText, pWeapon->GetWeaponName());
			Render::Text(size.x + (size.w / 2) - (nameSize.right / 2), size.y + size.h + 5, Color(255, 255, 255, 230), Render::Fonts::MenuText, pWeapon->GetWeaponName());
		}
...
void CEsp::DrawAmmo(IClientEntity* pEntity, Box size)
{
	...
	Color arc = Color(options::menu.visuals_tab.Ammo2.GetValue());
...
void CEsp::ammo_text(IClientEntity* pEntity, Box size)
{
	...
	if (pWeapon->isZeus27() || pWeapon->IsMiscGAY())
		return;
	bool text_side = options::menu.visuals_tab.OptionsName.GetState();
	CSWeaponInfo* weapInfo = pWeapon->GetCSWpnData();
	...
	static RECT Size = Render::GetTextSize(Render::Fonts::MenuText, "Hi");
	char ammoBuffer[512];
	//sprintf_s(ammoBuffer, "%d bullets", pWeapon->GetAmmoInClip());
	//Render::Text(size.x + size.w + 5, (size.y + (text_side ? 33 : 14)) + (1 * Size.bottom) - dist, Color(255, 255, 255, 255), Render::Fonts::MenuText, ammoBuffer);
...
void CEsp::DrawPlayer(IClientEntity* pEntity, player_info_t pinfo)
{
	...
	if (!teammate && options::menu.visuals_tab.OffscreenESP.GetState())
		enemy_out_of_range(pEntity, Color(options::menu.visuals_tab.Offscreen2.GetValue()));
	...
	if (get_box(pEntity, box, options::menu.visuals_tab.Active.GetState()) && (!teammate))
	{
		...
		if (options::menu.visuals_tab.OptionsBox.GetIndex() != 0)
		{
			...
			{
			case 1: default_box(box, options::menu.visuals_tab.BoxCol2.GetValue());
				break;
			case 2: bracket_box(box, options::menu.visuals_tab.BoxCol2.GetValue());
				break;
			case 3: corner_box(box, options::menu.visuals_tab.BoxCol2.GetValue(), pEntity);
				break;
		...
		{
			switch (options::menu.visuals_tab.OptionsName.GetState())
			{
	...
	if (get_box(pEntity, box, options::menu.visuals_tab.Active.GetState()) && options::menu.visuals_tab.show_team.GetState() && (teammate))
	{
		...
		if (options::menu.visuals_tab.OptionsName.GetState() != 0)
		{
			switch (options::menu.visuals_tab.OptionsName.GetState())
			{
			case 1: DrawName(pinfo, box, pEntity);
					...
					if (pTarget)
					{
						if (interfaces::engine->GetPlayerInfo(pTarget->GetIndex(), &pinfo2))
						{
							if (strlen(pinfo.name) > 16)
							{
								...
								Render::Text(scrn.right - TextSize.right - 4, (scrn.top / 4) + (16 * ayy), pTarget->GetIndex() == pLocal->GetIndex() ? Color(255, 255, 255, 255) : Color(255, 255, 255, 255), Render::Fonts::MenuText, pinfo.name);
								ayy++;
							}
						}
					}
...
void CEsp::DrawGun(IClientEntity* pEntity, CEsp::ESPBox size)
{
	...
	if (cClass)
	{
		...
		RECT nameSize = Render::GetTextSize(Render::Fonts::MenuText, meme.c_str());
		Render::Text(size.x + (size.w / 2) - (nameSize.right / 2), size.y + size.h + 1,
			Color(255, 255, 255, 255), Render::Fonts::MenuText, meme.c_str());
	}
}
...
void CEsp::DrawName(player_info_t pinfo, Box size, IClientEntity* pEntity)
{
	...
	{
		RECT nameSize = Render::GetTextSize(Render::Fonts::MenuText, pinfo.name);
		Render::TEXTUNICODE(size.x + (size.w / 2) - (nameSize.right / 2), size.y - 13, pinfo.name, Render::Fonts::MenuText, Color(255, 255, 255, 255));
	}
...
void CEsp::hp_text(IClientEntity* pEntity, Box size)
{
	...
	float dist = DistanceTo(vecOriginLocal, vecOrigin) / 3;
	//Render::Text(size.x + size.w + 5, (size.y + (text_side ? 1 : 0)) + (1 * Size.bottom) - dist, Color(10, 255, 170, 255), Render::Fonts::MenuText, HPBuffer);
...
void CEsp::ar_text(IClientEntity* pEntity, Box size)
{
	...
	Render::Text(size.x + size.w + 5, (size.y + (text_side ? 21.5 : 7)) + (1 * Size.bottom) - dist, Color(10, 170, 255, 255), Render::Fonts::MenuText, armorbuffer);
...
void CEsp::name_side(player_info_t pinfo, Box size, IClientEntity* pEntity)
{
	...
	if (MultiByteToWideChar(CP_UTF8, 0, pinfo.name, -1, buffer, 36) > 0)
	{
		Render::TEXTUNICODE(size.x + size.w + 5, size.y + (1 * Size.bottom) - dist, pinfo.name, Render::Fonts::MenuText, Color(options::menu.visuals_tab.NameCol2.GetValue()));
	}
}
...
void CEsp::DrawInfo(IClientEntity* pEntity, Box size)
{
	...
	bool text_side = options::menu.visuals_tab.OptionsName.GetState();
	float dist = DistanceTo(vecOriginLocal, vecOrigin) / 3;
	static RECT Size = Render::GetTextSize(Render::Fonts::Default, "e");
	if (entity->GetFlashDuration() > 0)
		Render::Text(size.x + size.w + 3, (size.y + (text_side ? 1 : 0)) + (1 * Size.bottom) - dist + 3.3, Color(66, 134, 244, 225), Render::Fonts::MenuText, "FLASHED");
		//stored_info.push_back(std::pair<std::string, Color>("Flashed", Color(30, 250, 30, 225)));
	if (BombCarrier == entity)
		Render::Text(size.x + size.w + 3, (size.y + (text_side ? 1 : 0)) + (1 * Size.bottom) - dist + 3.3, Color(255, 17, 17, 225), Render::Fonts::MenuText, "BOMB");
		//stored_info.push_back(std::pair<std::string, Color>("Bomb", Color(250, 17, 17, 225)));

	int i = 0;
	for (auto Text : stored_info)
	{
		RECT TextSize = Render::GetTextSize(Render::Fonts::MenuText, Text.first.c_str());
		Render::Text(size.x + size.w + 1, (size.y + 30) + i - ((TextSize.bottom / 2) - 5), Text.second, Render::Fonts::MenuText, Text.first.c_str());
		i += 8;
	}
...
{
	RECT defSize = Render::GetTextSize(Render::Fonts::MenuText, "");
	std::vector<std::string> Info;

	// Player Weapon ESP
	IClientEntity* pWeapon = interfaces::ent_list->GetClientEntityFromHandle((HANDLE)pEntity->GetActiveWeaponHandle());
	static RECT Size = Render::GetTextSize(Render::Fonts::Default, "Hi");
	RECT named = Render::GetTextSize(Render::Fonts::MenuText, "SCOPED");
	...
	if (options::menu.visuals_tab.OptionsInfo.GetState())
	{
		...
		RECT nameSize = Render::GetTextSize(Render::Fonts::MenuText, hp);
		bool text_side = options::menu.visuals_tab.OptionsName.GetState();
		float dist = DistanceTo(vecOriginLocal, vecOrigin) / 3;
		if (options::menu.visuals_tab.OptionsHealth.GetIndex() > 2 || options::menu.visuals_tab.OptionsArmor.GetIndex() > 2)
		{
			if (pEntity->ArmorValue() > 0)
			{
				...
				if (pEntity->HasHelmet())
					...
					//		Render::TEXTUNICODE(size.x + (size.w / 2) - (nameSize.right / 2), size.y - 8, pinfo.name, Render::Fonts::nnbruda, Color(255, 255, 255, 255));
					Render::Text(size.x +3 + size.w, (size.y + (text_side ? 1 : 0)) + (1 * Size.bottom) - dist - 6, Color(255, 255, 255, 225), Render::Fonts::MenuText, "HK");
				else
					Render::Text(size.x +3 + size.w, (size.y + (text_side ? 1 : 0)) + (1 * Size.bottom) - dist - 6, Color(210, 210, 210, 225), Render::Fonts::MenuText, "K");
			}
		}
	...
	if (options::menu.visuals_tab.OptionsInfo.GetState())
	{
		...
		if (pEntity->IsScoped())
		{
			Render::Text(size.x + size.w + 3, (size.y + (text_side ? 1 : 0)) + (1 * Size.bottom) - dist + 3.3, Color(66, 134, 244, 225), Render::Fonts::MenuText, "SCOPED");
		}
	}
	...
	for (auto Text : Info)
	{
		Render::Text(size.x + size.w + 3, size.y + (i*(Size.bottom + 2)), Color(255, 255, 255, 255), Render::Fonts::MenuText, Text.c_str());
		i++;
	}
}
...
void CEsp::DrawDrop(IClientEntity* pEntity, ClientClass* cClass, Box size)
{
	Color color;
	Box Box;
	...
	if (Weapon && !plr)
	{
		if (get_box(pEntity, Box, options::menu.visuals2_tab.FiltersWeapons.GetState()))
		{
			if (options::menu.visuals2_tab.FiltersWeaponsBox.GetState())
			{
				default_box(Box, Color(255, 255, 255, 255));
			}
			RECT TextSize = Render::GetTextSize(Render::Fonts::MenuText, Weapon->GetWeaponName());
			Render::Text(Box.x + (Box.w / 2) - (TextSize.right / 2), Box.y + Box.h + 5, Color(255, 255, 255, 255), Render::Fonts::MenuText, Weapon->GetWeaponName());
		}
	}
...
void CEsp::DrawSkeleton(IClientEntity* pEntity)
{
	...
	for (int j = 0; j < pStudioHdr->numbones; j++)
	{
		...
		if (pBone && (pBone->flags & BONE_USED_BY_HITBOX) && (pBone->parent != -1))
		{
			...
			{
				Render::Line(sParent[0], sParent[1], sChild[0], sChild[1], Color(options::menu.visuals_tab.Skeleton2.GetValue()[0], options::menu.visuals_tab.Skeleton2.GetValue()[1], options::menu.visuals_tab.Skeleton2.GetValue()[2], options::menu.visuals_tab.Skeleton2.GetValue()[3]));
			}
		}
	}
}
...
void CEsp::BoxAndText(IClientEntity* entity, std::string text)
{
	Box Box;
	std::vector<std::string> Info;
	RECT nameSize = Render::GetTextSize(Render::Fonts::MenuText, "");
	if (get_box(entity, Box, options::menu.visuals2_tab.FiltersNades.GetState()))
	{
		Info.push_back(text);
		if (options::menu.visuals2_tab.FiltersNades.GetState())
		{
			int i = 0;
			for (auto kek : Info)
			{
				default_box(Box, Color(255, 255, 255, 255));
				Render::Text(Box.x + (Box.w / 2) - (nameSize.right / 2), Box.y - 11, Color(255, 255, 255, 255), Render::Fonts::MenuText, kek.c_str());
				i++;
			}
		}
	}
...
void CEsp::enemy_out_of_range(IClientEntity *player, Color color)
{
	...
	if (!GameUtils::World2Screen(pos_3d, pos) || !GameUtils::World2Screen(top_3d, pos))
		draw_direction_arrow(screen_center, at_target_yaw.y - v_real_angles.y, radius, length, color);
}